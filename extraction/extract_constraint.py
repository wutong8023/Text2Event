...
def get_constraint_decoder(tokenizer, type_schema, decoding_schema, source_prefix=None):
    ...
    elif decoding_schema == 'iterative_treespan':
        return IterTreeConstraintDecoder(tokenizer=tokenizer, type_schema=type_schema, source_prefix=source_prefix)
    else:
...
class IterTreeConstraintDecoder(ConstraintDecoder):
    """
    iterative tree generation:
        once get a subtree of triplet (r -> e1 -> e2), then regenerate this triplet from right to left (e2 -> e1 -> r)
    """
    def __init__(self, tokenizer, type_schema, *args, **kwargs):
        super().__init__(tokenizer, *args, **kwargs)
        self.tree_end = '<tree-end>'
