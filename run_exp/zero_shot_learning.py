...
def get_cmd():
    ...
    for tuning_type in ["prefix", "both", "fine"]:
        ...
        for source_data in ["oneie/few-shot_23_test_10", "oneie/oneie_23_training"]:
            current_time = datetime.now().strftime('%Y-%m-%d-%H-%M')
            source_output_dir = f"models/zsl_{tuning_type}_{source_data.split('/')[1]}_{current_time}"
            ...
            for model_name in ["t5-base"]:
                for epoch in [60]:
                    ...
                    info_list.append(f"zsl_{tuning_type}_{source_data.split('/')[-1]}")
                    cmd_list.append(cmd)
            # transfer learning
            """
